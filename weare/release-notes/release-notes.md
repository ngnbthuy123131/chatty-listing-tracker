...
• Mô tả:
    ◦ Cập nhật Product List View để hiển thị toàn bộ sản phẩm từ Shopify, với 2 trạng thái:
        ▪ Active: sản phẩm đang được sync và sử dụng cho AI training (theo plan limit sau khi cài app).
        ▪ Inactive:
            • Sản phẩm đã sync nhưng bị tắt, không dùng cho AI training.
            • Sản phẩm mới được thêm từ Shopify, cần nhấn Resync để cập nhật vào danh sách.
    ◦ Merchant có thể search / filter sản phẩm và thực hiện bulk action để chuyển trạng thái Active / Inactive nhanh chóng.
Note: hiện tại label đang hơi khó hiểu chút, team đang sửa lại ạ
...
### 2. Improve FAQs translation (Bỏ Updated status)
...
Các bước connect:
    ...
    ◦ Nhấn Allow → redirect về app và hiển thị status Connected 
Sau khi kết nối
 ◦ Tự động tạo ticket trên Zendesk khi conversation được resolve https://capture.avada.io/i/r5qwbX9J5kjX
◦ Nội dung ticket bao gồm:
        ▪ Transcript hội thoại
        ▪ Thông tin khách hàng (user profile)
...
### 4. [Improvement] Gợi ý câu hỏi Test AI theo bộ câu hỏi chung
...
• Mô tả:
    ...
    ◦ Merchant có thể chọn chủ đề test, hệ thống sẽ hiển thị danh sách câu hỏi gợi ý tương ứng.
    ◦ Các câu hỏi được thiết kế ngắn gọn, generic, bám keyword e-commerce, click để auto-send và không chứa thông tin đặc thù của bất kỳ store/ngành hàng nào.
...
### 5. Related conversation tại inbox
...
▪ Customer: các conversation thuộc cùng customer profile (bao gồm login, reset, lead được convert)
▪ Lead: các conversation có cùng email/phone hoặc reset trong cùng session
▪ Anonymous: các conversation được reset trong cùng session
◦ Mặc định hiển thị 5 conversation gần nhất, scroll để xem nhiều hơn
◦ Sắp xếp theo thời gian tin nhắn gần nhất, ưu tiên cùng session
◦ Mỗi related conversation hiển thị thông tin về customer name, last message, Last message sent time, Conversation status, Assignee
◦ Click vào item để mở trực tiếp conversation tương ứng

**Task:** [Inbox] Related conversation  (https://www.notion.so/Inbox-Related-conversation-2e5b0da449f180918423f161c7ce1a85?pvs=21)

---

### 6. Tạo default FAQs theo ngôn ngữ lựa chọn tại Onboarding
...
• Mô tả:
    ◦ Sử dụng ngôn ngữ MC chọn trong Onboarding để:
        ▪ Tạo danh sách FAQs mặc định theo ngôn ngữ tương ứng
        ▪ Tự động đặt làm Default language trong Settings → Translation
    ◦ Trường hợp MC chọn Others hoặc không chọn ngôn ngữ, mặc định sử dụng English
...
### 7. [Improvement] Hiển thị Discount đúng đối tượng khách hàng
...
• Mô tả:
    ...
    2. Lọc discount theo Eligibility khi AI gợi ý
        ▪ AI chỉ hiển thị mã giảm giá phù hợp với từng khách, dựa trên eligibility đã set up trên Shopify
        ▪ Lead / Anonymous: chỉ thấy discount áp dụng cho All customers
        ▪ Customer: chỉ thấy discount đúng với segment hoặc customer cụ thể
• Giá trị mang lại:
    ◦ Tránh nhầm lẫn và phàn nàn về mã giảm giá.
    ◦ Tăng trải nghiệm cá nhân hoá và khả năng chốt đơn.
...
### 9. [Improvement] Cập nhật hiển thị Additional AI Conversation Charge
...
▪ Chỉ cập nhật khi đạt mốc tính bill (ví dụ $10), hoặc kết thúc billing cycle 
▪ Hiển thị billing period từ ngày bắt đầu cycle đến ngày tính bill 
▪ Không hiển thị realtime số conversation đã dùng trong kỳ
→ Ảnh: https://capture.avada.io/i/iCGm5cOBBtgB
2. Cập nhật Subscription
▪ Hiển thị tổng additional AI conversations và additional charge theo tháng (không reset mỗi lần tạo bill)
...
### 10. [Inbox menu] Improve insert link
...
### 11. Refactor code proactive chat
...
•  Mô tả: 
    ◦ Refactor luồng gửi message cho các proactive chat sau: 
        ...
        ▪ Subscribe newsletter: đồng bộ theo luồng Quick question 
        ▪ Abandoned cart reminder và search page: cập nhật code để theo chuẩn chung 
        ▪ Các proactive chat Product Recommendation / Cart Booster / Browse Collection giữ nguyên 
    ◦ Áp dụng luồng gửi message thống nhất cho tất cả proactive chat (từ Subscribe Newsletter trở đi), xử lý rõ ràng theo từng cấu hình Pre-chat (Guest / Anonymous / Both options) 
    ◦ Đảm bảo message được gửi đúng thứ tự, đúng vai trò (customer/admin) và AI phản hồi chính xác khi được bật
...
### 12. Improve trả lời câu hỏi về collection
...
2. Hỏi về collection chung (không nêu trend / loại sản phẩm)
• AI sắp xếp sản phẩm theo thứ tự ưu tiên: Best seller → New arrival → Sản phẩm còn lại.
• Trả về danh sách sản phẩm tổng hợp với số lượng phù hợp để gợi ý.
...
### 13. Định nghĩa conversation tính vào usage
...
• Mô tả: AI usage được tính theo conversation, không phụ thuộc vào số lượng message
    ...
    ◦ Conversation kết thúc khi:
       ◦ Resolve (auto/manual) 
       ◦ Reset 
       ◦ Session expired (5 ngày, áp dụng với cả customer, kết thúc kể cả khi status = Open) 
       ◦ Delete (kết thúc tại thời điểm xoá) 
       ◦ Block manual (kết thúc tại thời điểm block) 
    ◦ Conversation chỉ được tính usage khi có ít nhất 1 AI reply hợp lệ (có source)
    ◦ Các conversation không có AI reply, chỉ là teaser proactive, hoặc message hệ thống sẽ không bị tính usage
    ◦ Với proactive chat, chỉ tính usage khi có tương tác 2 chiều thực sự giữa khách và AI (có ít nhất 1 reply từ customer)
...
### 14. Thêm email thông báo khi đạt monthly usage (trường hợp spending limit > 0)
...
• Nội dung email:
    ◦ Thông báo AI bắt đầu phát sinh chi phí
    ◦ Gợi ý next action: Upgrade plan / Điều chỉnh spending limit / Review usage details (https://capture.avada.io/i/aVwDBsNTRgoQ, https://capture.avada.io/i/QiUf8Vzngp0C)
...
### 15. Update onboarding segment tệp khách hàng
...
• How AI transfers to your team: Setup luồng AI → Human (áp dụng trực tiếp vào settings trong app)
• Try these use cases: Hiển thị các kịch bản AI phổ biến (product recommendation, order tracking, return…)
Với Human
• Your brand, your tone (Chatbox): cài đặt giao diện chatbox: color, welcome message, language
• What chatbox can help customers: Setup nhanh các block hiển thị trên chatbox (Contact us, Order tracking, FAQs, Featured category…)

Video: https://somup.com/cOVuQTSsxt
...
### 16. [New Feature] Gợi ý Complementary Products trong Proactive Chat
...
• Mô tả:
    ...
    ◦ Tự động loại trừ các sp: hết hàng, giá = 0, gift card và sản phẩm đã có trong giỏ hàng
    ◦ Ưu tiên danh sách complementary products từ app Shopify Search & Discovery (nếu store đã cài)
...
### 17. [Joy integration] Require ultimate plan
...
• Mô tả:
    1. Kiểm tra plan JOY trước khi kết nối (Step 2):
        ▪ JOY Ultimate: bật nút Connect → nhấn Connect, trạng thái hiển thị Connected.
        ▪ Không phải Ultimate: khi nhấn Connect → hiển thị banner yêu cầu nâng cấp JOY.
        ▪ Bỏ modal khi Connect/Disconnect (chuyển sang trạng thái trực tiếp).
    2. Cập nhật nội dung hiển thị:
        ▪ Chỉnh sửa mô tả và ảnh preview của integration.
        ▪ Bổ sung mô tả về AI có thể trả lời thông tin loyalty khi kết nối thành công
...
• Mô tả:
    ...
    ◦ Nhấn Create để tạo conversation mới:
        ▪ Profile conversation được tạo là lead hoặc customer 
        ▪ Assignee mặc định là người nhấn tạo conversation
    ◦ Khi gửi tin nhắn:
        ▪ Gửi email đến khách hàng và các participants
        ▪ Email có reply-to gắn với conversation ID, giúp mọi phản hồi qua email được gửi vào đúng conversation trong Inbox
...
### 19. Update AI assistant list view layout
...
• Mô tả:
    ◦ Cập nhật layout trang AI:
        ▪ Activate AI chuyển lên page action:
            ...
            • Click Deactivate → hiển thị modal xác nhận dừng AI và tiếp tục dùng live chat
        ▪ Unresolved questions chuyển từ card thành table hiển thị tại màn list, chỉ hiển thị khi có dữ liệu
        ▪ Test chuyển từ page action xuống dưới dạng card
    ◦ Thêm preview card thay cho icon để trực quan và dễ nhận biết hơn
...
• Mô tả: Tách text field General Instruction thành các section rõ ràng, có cấu trúc gồm: 
        1. Role: xác định vai trò của AI trong hội thoại
        2. Tone of voice: lựa chọn 
        3. Custom Instruction: Thêm hướng dẫn nghiệp vụ riêng cho store
        4. Conversation boundaries: Quy định phạm vi nội dung AI được/không được trả lời.
    ◦ Giúp merchant dễ hiểu, dễ cấu hình và tránh nhập instruction trùng lặp hoặc không phù hợp
...
### 21. Improve UI scenario size guide
...
### 22. [Analytics] Agent performance report
...
• Mô tả: bổ sung 2 chỉ số: Handling Time và Resolution Time
    ...
    ◦ Resolution time: thời gian từ lúc khách gửi tin nhắn đầu tiên đến khi conversation được resolve, phản ánh tốc độ giải quyết vấn đề
...
### 23. [Improvement] Thêm tuỳ chọn Subscribe trên Pre-chat form
...
• Mô tả:
    ◦ Thêm tuỳ chọn Subscribe trên Pre-chat Form
        ▪ Chỉ hiển thị khi chọn How to start chat = Chat as guest/Show both option
        ▪ Cho phép customize nội dung thông báo subscribe
    ◦ Tại storefront: khi khách tích Subscribe và submit form, kiểm tra email:
        ▪ Tồn tại: cập nhật status Subscribed nếu trước đó là Unsubscribed
        ▪ Chưa tồn tại: tạo mới customer với status Subscribed
...
### 24. [Improvement] Thêm chỉ số Chatty Contributed Sales
...
### 25. [Improvement] Sync cart updates với custom theme
...
• Mô tả:
    ◦ Bổ sung custom event của Chatty: chatty:cart:changed để theme có thể lắng nghe và refresh cart UI khi cart thay đổi
• Cách thực hiện:
    ◦ Mở file theme
    ◦ Thêm script của Chatty vào custom theme
    ◦ Script sẽ tự động trigger refresh cart UI mỗi khi cart có thay đổi

Link helpdesk: https://help.meetchatty.com/ai/overview/what-ai-assistant-can-do/syncing-cart-updates-with-custom-themes

---

### 26. [Improvement] Nâng trải nghiệm trial với conversation bonus & product limit
...
• Mô tả:
    ◦ Trong thời gian trial (Basic/Pro):
        ...
        ▪ Tăng product limit lên 10,000 trong khi trial
    ◦ Sau khi kết thúc trial:
        ▪ Ẩn banner và thông tin conversation bonus
        ▪ Tự động cập nhật product limit về đúng plan đã chọn: giữ lại các sp mới nhất theo plan limit, tự động disable/remove các sp vượt limit
        ▪ Hiển thị banner thông báo hết trial để hướng dẫn MC upgrade
...
• Mô tả:
    ...
    ◦ Message: Tự động sử dụng template phù hợp theo loại nội dung:
        ▪ Text: custom/quick question 
        ▪ Product recommendation, Coupon: sẽ bổ sung thêm sau
...
### 29. Bổ sung các loại message cho proactive chat
...
### 30. [Design] Update UI các form trên inbox và storefront
...
### 31. Improve proactive chat design settings
...
### 32. Filter sản phẩm gợi ý còn hàng ở proactive chat
...
### 33. Thêm default proactive chat cho store mới
...
### 34. [Improvement] Report Issue cho AI Response
...
• Mô tả:
    ...
    ◦ Khi merchant gửi feedback, hệ thống tự động đẩy message vào Crisp chat để team support xử lý nhanh
    ◦ Conversation tại Crisp được gắn tag chatty-feedback để dễ filter và theo dõi
...
### 35. Đổi domain email forwarding
...
• Mô tả:
    ...
    ◦ Giúp tránh việc email bị reject bởi Cloudflare, đảm bảo email forwarding hoạt động 
• Lưu ý:
    ◦ User đã kết nối email Outlook bị lỗi cần xoá email và connect lại 
    ◦ Không ảnh hưởng đến việc gửi email từ Chatty
...
• Mô tả: Thêm scenario Inventory status cho phép tùy chỉnh cách AI trả lời theo trạng thái tồn kho: In stock, Out of stock, Available on backorders
    ◦ Để trống: AI sử dụng luồng trả lời mặc định
    ◦ Nhập instruction: override instruction mặc định
...
### 37. Custom after-sales support scenario
...
• Mô tả: Bổ sung settings cho scenario After-sales Service gồm:
    ◦ Hướng dẫn merchant thêm training data liên quan về policy 
    ◦ Lựa chọn cách handover theo từng case cụ thể, gồm: Return & refund, Cancel order, Order issue, Edit order 
    ◦ Các phương thức handover 
        ▪ Transfer to email: Hiển thị after-sales for để thu thập thông tin, tùy chỉnh field trên form (email, phone, issue description, order number, photo upload), conversation được resolve sau khi khách submit form
        ▪ Transfer to human (Chatty Inbox): Tự động reassign conversation theo thiết lập Assignment Automation
        ▪ Send contact form information: hiển thị contact methods cài đặt tại Chatbox

**Task:** Custom scenario after sales (https://www.notion.so/Custom-scenario-after-sales-23fb0da449f1803491cac959f7d885dc?pvs=21)

---

### 38. Đọc ảnh sản phẩm trong tin nhắn từ các channel
...
• Mô tả:
    ...
    ◦ Hiển thị banner theo 2 tệp KH:
        ▪ MC có Avg monthly order > 40 https://capture.avada.io/i/TeKwbJcKNgC4
            • MC click button → mở LinkedIn và tự động gửi Crisp message: “Hi there. Just followed Chatty on LinkedIn. Excited to grab that 40% discount!”
            • CS xin ảnh chụp follow từ khách
            • CS gửi discount LINKEDIN-FOLLOWER-40OFF-1MO
        ▪ Các MC còn lại: https://capture.avada.io/i/MtJ1Un2MOeY2
            • Click button → mở trang LinkedIn (ko áp dụng discount)
...
### 40. Thêm AI thinking state khi AI trả lời
...
### 41. Update quy tắc hiển thị preview product đang xem
...
• Mô tả:
    ◦ Cập nhật quy tắc hiển thị product preview theo ngữ cảnh người dùng:
        1. Auto preview theo product page
            • Khi mở chatbox từ trang sản phẩm, hiển thị preview của sản phẩm đang xem.
            • Preview được giữ nguyên cho đến khi khách chuyển trang hoặc đóng preview
        2. Preview theo proactive chat gợi ý sản phẩm
            • Khi khách click teaser proactive chat, hệ thống tự động gửi câu hỏi về sản phẩm được gợi ý.
            • Đồng thời chuyển preview sang sản phẩm được gợi ý, thay vì sản phẩm đang xem trước đó
...
• Mô tả:
    ...
    2. Bảng Assisted Orders chi tiết
        ▪ Danh sách đơn hàng có sự tương tác với Chatty
        ▪ Thông tin bao gồm:
            • Order ID
            • Order Date
            • Customer name (click để xem danh sách conversation liên quan)
            • Order Amount
...
### 43. Cập nhật rule hiển thị lại proactive chat
...
• Mô tả:
    ...
    ◦ Product Recommendation / Collection Boost
        ...
        ▪ Khi khách nhấn Close ở trang nào, teaser chỉ bị ẩn tại trang đó và vẫn hiển thị ở các trang khác
    ◦ Cart Booster: luôn hiển thị khi khách add to cart và thỏa mãn điều kiện cart value
...
### 44. Improve proactive chat design settings
...
### 45. Dịch email message admin gửi cho customer
...
### 47. [Subscribe newsletter] Thêm CTA trên teaser
...
• Bổ sung field Trigger button text để tùy chỉnh label CTA trên teaser
• Gồm các field: Introduction, Form fields, Marketing double opt-in
2. Thêm CTA “Yes, sure!” trên teaser
• Click CTA → mở chat và hiển thị teaser message
• Tự động gửi tin nhắn từ phía khách: “Yes, sure!”
• Tạo conversation với contact type = Anonymous
• Gửi subscribe form theo settings đã cấu hình
3. Thêm card Appearance: cài đặt màu teaser
...
### 48. [New Feature] Integration với Powerful Contact Form
...
- Cài đặt email forwarding là email dùng để nhận thông tin form submission
- Khi khách submit form, inbox tự động tạo conversation mới với customer profile lấy dữ liệu từ form và nội dung tin nhắn là thông tin được submit (trên form bắt buộc phải có email/phone) 
- Email thông báo form sẽ không được tạo tại inbox → tránh tạo conversation lặp
...
### 49. Block conversation theo IP customer
...
• Mô tả:
    ...
    ◦ Sau khi bị block IP, các conversation mới từ IP đó sẽ được tự động chuyển vào mục Blocked, user vẫn có thể gửi tin nhắn nhưng reload sẽ ẩn
...
### 50. Bỏ setting “Send transcript history”
...
• Mô tả:
    ...
    ◦ Thêm event log tại Inbox khi khách gửi transcript từ chatbox
    ◦ Với store cũ (dù tắt transcript) cũng mặc định hiện nút Send transcript trên chatbox
...
### 51. [Proactive: Product recommendation] Thêm quick message button (custom, auto)
...
• Quick question: mặc định thêm 3 câu, cho phép thêm sửa xoá, max 10 câu hỏi. Khách click vào một câu hỏi → tự động gửi câu hỏi https://capture.avada.io/i/SIAcBdC0vb1D
...
### 52. [Improvement] Tự động mark as read cho resolved conversation
...
• Mô tả:
    ...
    ◦ Sau khi lựa chọn, hệ thống tự động cập nhật tất cả label và mô tả trên app sang ngôn ngữ đã chọn

**Task:** Translation in app  (https://www.notion.so/Translation-in-app-07efa4a7ac96409197d2c110217cc470?pvs=21)

---

### 55. [Shopify flow] Tạo trigger create customer
...
A. Thêm event “Pre-chat Submitted” cho Shopify Flow
    ...
    ◦ Tích hợp với app marketing (ví dụ: Omnisend) để:
        ▪ Trigger welcome campaign
        ▪ Tạo segment Chatty dựa trên tag để gửi campaign
        ▪ Cập nhật profile khách hàng
https://capture.avada.io/i/OA6UJ4CCThuN
B. Thêm integration Shopify Flow: hiển thị hướng dẫn cách sử dụng Chatty event trong Flow
...
### 56. Export conversation to CSV, TXT
...
Nội dung file gồm: thời gian gửi/nhận, người gửi, nội dung tin nhắn: https://capture.avada.io/i/urIBIfso3eom, https://capture.avada.io/i/xJcZuqpyBLwS
...
### 57. [Improvement] Thêm embed float chatbox button
...
### 58. [New feature] Tạo script cho FAQs block
...
### 59. [Improvement] Thêm điều kiện Maximum Cart Value cho Cart Booster
...
Mô tả:
• Bổ sung điều kiện mới: Maximum Cart Value https://go.screenpal.com/watch/cTlnQfnqdby
• Proactive chat chỉ hiển thị khi giá trị giỏ hàng nằm trong khoảng:
Minimum value ≤ Cart amount ≤ Maximum value
...
### 60. Proactive chat “Cart reminder”
...
• Mô tả:
    ...
    ◦ Điều kiện hiển thị:
        ▪ Áp dụng các điều kiện tương tự proactive chat hiện có — audience, device, display time, display duration
    ◦ Message: Tự động sử dụng template phù hợp theo loại nội dung:
        ▪ Text: hiển thị teaser từ admin
        ▪ Product recommendation: gửi tin nhắn gợi ý sản phẩm từ admin
        ▪ Coupon: gửi message từ phía khách để trigger luồng giảm giá
...
### 61. Thêm hướng dẫn cài đặt mobile app tại Inbox
...
### 64. Thêm button Ask about it trên product card
...
1. Bật AI:
    ...
    ◦ View similar: gửi tin nhắn gợi ý sản phẩm tương tự → mở rộng lựa chọn để so sánh 
    ◦ View product: click vào tên sản phẩm để mở trang sản phẩm 
2. Tắt AI:
    ◦ View product: mở trang sản phẩm trong tab mới
    ◦ Ask about it: mở chatbox, không gửi tin nhắn
...
### 65. [New Feature] Quick Question cho Welcome Visitor
...
4. Tự động gửi câu hỏi vào chat, tạo conversation 
5. AI reply (nếu enable) → hiện pre-chat form 
    ◦ Cập nhật rule hiển thị proactive chat: nếu nhiều proactive chat cùng loại thoả điều kiện trên 1 trang → chỉ hiển thị chat có ưu tiên cao nhất
...
### 66. [New feature] Auto tag conversation
...
• Mô tả 
    ...
    ◦ Thêm filter conversation by Tag trong Conversation Inbox https://prnt.sc/m9pNtB2CXX4_
...
### 67. [New feature] Rewrite message in inbox
...
### 68. Thêm filter, search tại modal select product
...
• Message = Custom product recommendation: nhanh chóng chọn sản phẩm để gợi ý
2. AI Assistant → Data source → Smart collection
• Khi thiết lập smart collection (Best Seller, New Arrival, Sales…), merchant có thể dùng bộ lọc để chọn sản phẩm chính xác và nhanh hơn
...
3. Generate câu hỏi phù hợp dựa trên thông tin gap
...
### 70. [New Feature] Advanced spam filter
...
- Khi bật: hệ thống tự động phân tích nội dung tin nhắn và chặn cuộc hội thoại bị xác định là spam
• Conversation sẽ được đánh dấu “Blocked” và chuyển sang folder Spam
• Người gửi vẫn có thể nhắn, nhưng tin nhắn sẽ không hiển thị trong Inbox
• Merchant có thể xem lại trong Spam và Unblock
...
### 71. Tạo conversation riêng khi gửi qua contact form trên store
...
- Gắn đúng email khách hàng vào conversation bằng cách dùng email reply-to ở email làm sender của conversation https://prnt.sc/_B1_d1dolTHo
...
### 72. Thêm insert video ở menu inbox
...
Preview & hiển thị video trong đoạn chat
Sau khi gửi, video sẽ hiển thị
• Thumbnail preview
• Thời lượng video
• Nút Play, chỉnh volume, speed, xem trực tiếp trong chat
• Nút Download để tải video
Lưu trữ video theo plan (tương tự ảnh, file)
• Free plan: Upload lên Shopify
• Paid plan: Upload lên CDN
...
### 73. [New Feature] AI đọc & phân tích ảnh khách hàng gửi qua chat
...
3. Trả lời dựa trên tin nhắn và ảnh khách gửi
3. Các trường hợp AI xử lý
• Gợi ý sản phẩm tương tự từ ảnh khách gửi
• Thông báo nếu ảnh không liên quan hoặc không đủ thông tin
• Một số tình huống chưa hỗ trợ (sẽ tiếp tục improve): tư vấn phối đồ, khiếu nại, đổi trả…
...
### 74. [New feature] AI đọc thông tin từ Joy loyalty
...
• Khi connect Joy Loyalty:
    ...
    ◦ Đủ điều kiện: hiển thị badge AI support enabled xác nhận AI hỗ trợ trả lời câu hỏi loyalty https://prnt.sc/YgwN7P16BUY9
2. AI đọc & xử lý dữ liệu từ Joy để trả lời khách
• Sử dụng API Joy để lấy thông tin:
    ◦ Customer loyalty profile
    ◦ Earning program
    ◦ Redeem / reward program
• AI được training để:
    ◦ Hiểu và dùng dữ liệu loyalty của từng khách
    ◦ Trả lời các câu hỏi liên quan đến điểm thưởng, tier, cách earn và redeem
Ví dụ câu hỏi AI có thể trả lời:
• “How many loyalty points do I have?”
• “What benefits do I get with my tier?”
...
• Giới hạn số ngôn ngữ được auto-translate theo plan:
    ...
    ◦ Plus: Unlimited
• Cập nhật giao diện:
    ◦ Bỏ card “Auto-translate” ở màn translation list view
    ◦ Khi nhấn Auto-translate lần đầu → đánh dấu “Auto-translated” và trừ vào usage
    ◦ Khi đạt limit → hiển thị banner cảnh báo và tự động disable nút Auto-translate
Note:
• Default language không áp dụng auto-translate
• Update/Downgrade: 
     • Downgrade: Paid → Free: bỏ auto translated cho tất cả ngôn ngữ, Pro/ Plus → Basic: giữ lại auto-translated của ngôn ngữ gần nhất 
     • Upgrade: ko tự động bật auto-translate
...
### 76. Thêm lựa chọn team member được assign
...
• Thêm cài đặt Assignment member trong phần auto-assignment:
    ◦ All members: assign cho toàn bộ member có trạng thái active
    ◦ Specific members: chọn các thành viên được assign
• Store cũ sẽ được mặc định ở All members để không ảnh hưởng đến setting hiện tại
...
### 77. Cung cấp đúng variant khi khách hỏi specific variant
...
• File được tự động xoá sau vài ngày khi shop uninstall, nhằm đảm bảo bảo mật dữ liệu
...
### 79. Cài đặt hiển thị online status trên chatbox
...
• To [Customer Name]: chỉ hiển thị với conversation từ customer
• To someone else: chho phép nhập địa chỉ email khác để gửi transcript
...
• Khi bật:
    ...
    ◦ Product card hiển thị dưới dạng link khi gửi qua WhatsApp
...
### 82. Thêm preview scenario “Transfer to human”
...
### 83. Sync order metafield cho AI training
...
• Action:
    ◦ Enable / Disable: chọn metafield dùng cho AI
    ◦ Re—sync: cập nhật lại danh sách order metafield từ Shopify
2. AI sử dụng Order Metafield để trả lời
• AI đọc dữ liệu từ order metafield để trả lời các câu hỏi liên quan đến đơn hàng
• AI verify order trước khi đưa thông tin (tương tự order tracking, KH cần gửi Order number và Phone/Email) 

Ví dụ: Metafield “Expected delivery date”
Khi khách hỏi về trạng thái đơn hàng, AI sẽ trả lời thông tin tracking kèm ngày giao hàng dự kiến đã điền ở order metafield https://prnt.sc/DyFlT7C0TdBF
...
### 84. Update settings AI theo agent
...
• Mô tả cập nhật 
    ...
    ◦ Instructions: 
        ▪ General instruction: giữ nguyên
        ▪ Shopping skill: chuyển từ Smart collection và scenario Size guide 
        ▪ Customer support skill: chuyển từ default scenario Handover, Order tracking và After-sales service 
        ▪ Custom scenario: giữ nguyên 
    ◦ Test AI: hiển thị ở tất cả các trang để mở test nhanh hơn
    ◦ Unresolved questions: giữ nguyên, thêm hiển thị số câu hỏi ở màn list (trước dùng banner)
...
### 85. Toggle push notification for online store only
...
### 87. [Improvement] Update footer email transcript
...
### 88. [Proactive chat] Gợi ý sản phẩm tương tự với sản phẩm đang xem
...
### 89. [New proactive chat] Browse collection page
...
• Mô tả: thêm template mới: “Browse collection page”
    ...
    ◦ Message type: default tin nhắn phù hợp với trang collection, gồm Message/Coupon/Product recommendation
...
### 90. Embed AI chat on product page
...
• Mô tả: thêm setting Embed product AI assistant
    ...
    ◦ MC thêm vào storefront bằng cách add block trên theme editor, cho phép customize định dạng https://prnt.sc/p3W5n_ZCm0Vk
    ◦ Khi KH mở trang sản phẩm, hiển thị 3–5 câu hỏi gợi ý kèm ô chat, câu hỏi gợi ý được AI tự động tạo dựa trên dữ liệu sản phẩm https://prnt.sc/cY32pOXDYBem
• User flow
    ◦ Customer mở product page
    ◦ AI phân tích thông tin sản phẩm → tạo 3–5 câu hỏi gợi ý
    ◦ Customer chọn câu hỏi hoặc nhập câu hỏi riêng
    ◦ Mở chatbox, gửi câu hỏi và kích hoạt AI
    ◦ AI trả lời dựa trên context của sản phẩm hiện tại
...
### 91. AI trả lời dựa vào sản phẩm đang xem
...
• Khi khách mở chat, hiển thị preview của sản phẩm đang xem trong ô nhập tin nhắn https://prnt.sc/3UfJc2hx1tXy
     ...
     • Click Add to cart để gửi tin nhắn thêm vào giỏ hàng 
     • Nhấn “Close” để đóng preview và xoá context sản phẩm
• Tự cập nhật context khi khách chuyển sang sản phẩm khác

Ví dụ:“Is this in stock?” “Does it have white color available?” “What sizes does this come in?” “Is this waterproof?”
...
### 92. Thêm điều kiện hiển thị proactive chat
...
### 93. Auto-assign cho online member
...
• Thêm tùy chọn “Online member” trong Assignment method → Auto Assign https://prnt.sc/0Rm1HPHFIPwt
    ...
    ◦ Nếu không có ai online → conversation được chuyển thành Unassigned và tự động assign lại khi có agent online
...
### 94. Custom setting for scenario “Transfer to human”
...
• Transfer method:
    ...
    ◦ Conditional transfer: chỉ chuyển trong working hour
    ◦ No transfer: Ko chuyển, AI luôn trả lời 
• Response customization:
    ◦ Pre-transfer / Post-transfer message: tuỳ chỉnh nội dung trước và sau khi chuyển.
    ◦ Show alternative contact methods : cài đặt các phương thức liên hệ hiển thị khi AI không transfer (Contact form / Contact methods)
• Continue with AI: chuyển từ cài đặt từ AI Settings
2. AI Settings
• How to transfer: bỏ
• Continue with AI: di chuyển sang scenario
...
• Mô tả tính năng
    ◦ Thêm scenario “Size guide”  https://prnt.sc/bRJLB6G2hzSt
        ▪ Cho phép tạo và quản lý size guide theo từng sản phẩm
        ▪ Limit: format jpg, png, 5 file, 5MB/file
    ◦ Settings instructions: hướng dẫn cách AI tư vấn ngoài luồng mặc định, customize tin nhắn khi sản phẩm không có size guide
    ◦ AI Response:
        ▪ AI có thể tự động gợi ý size khi khách hỏi về size
        ▪ Nếu có size guide: AI tư vấn dựa trên dữ liệu khách gửi
        ▪ Nếu không có: AI gửi fallback message
        ▪ Ví dụ câu hỏi khách hàng: What size should I choose for this shirt?”, “Do you have a size chart for this product?” “Is this dress true to size?”
...
### 96. [New Feature] Banner khuyến mãi BFCM Early Bird
...
• Điều kiện hiển thị:
    ...
    ◦ AI Assistant = ON
    ◦ Đang dùng monthly subscription 
• Chỉ hiển thị 1 lần, sau khi claim không hiển thị lại
...
### 97. [New feature] Sync variant metafield
...
• Enable / Disable để bật hoặc tắt việc sử dụng dữ liệu variant metafield cho AI training 
• Mặc định limit 10 variant metafield → hiện banner contact support khi đạt limit (CS có thể mở ở dev zone https://prnt.sc/LQNvtyCfbyQX) 
• Dữ liệu từ variant metafield được sử dụng cho AI trả lời câu hỏi liên quan đến từng variant
...
### 98. [Improvement] Thông báo reach đạt metafield
...
1. Cảnh báo khi sync > 10 metafields
    ...
    ◦ Disable action “Enable” khi đã bật đủ 10 metafields.
    ◦ Hiển thị banner cảnh báo “Metafield limit exceeded” kèm CTA “Contact us” để mở thêm limit.
2. Cập nhật mô tả metafield
    ◦ Thêm link guide hướng dẫn chi tiết về giới hạn và cách quản lý metafields.
Luồng CS hỗ trợ
1. Hỏi nhu cầu cụ thể của khách về số lượng metafield cần mở.
2. Mở số lượng metafield tương ứng trong Dev Zone (kèm ảnh hướng dẫn nội bộ)
...
### 99. [Data source] Thêm sync Shopify discount
...
• Thông tin được sync:
    ...
    ◦ Discount code
    ◦ Type (Percentage, Amount, Buy X Get Y...)
    ◦ Condition: market, customer, product, collection (tuỳ loại discount)
    ◦ Validity period (Start – End date)
    ◦ Usage frequency
• Tần suất sync: cập nhật real-time theo dữ liệu trên Shopify
2. AI response
AI nhận diện và phản hồi các câu hỏi liên quan đến khuyến mãi, bao gồm:
• Gợi ý discount phù hợp khi khách hỏi về ưu đãi hiện có
• Giải thích lợi ích, điều kiện, hoặc cách áp dụng discount
Example: 
• “Do you have any coupon codes right now?” → AI liệt kê các discount
• “Can I use this discount if I'm ordering from Canada?” → AI kiểm tra điều kiện áp dụng theo region
• “Does the sale apply to all products?” → AI xác định phạm vi sản phẩm/collection áp dụng
• “What discounts are available for this product?” → AI gợi ý ưu đãi cụ thể theo sản phẩm
...
### 100. Promote sản phẩm sales, occasion
...
• Search term: Cho phép thêm từ khóa để AI trigger gợi ý sản phẩm từ collection tương ứng.
• Product conditions: Chọn sản phẩm cụ thể hoặc toàn bộ sản phẩm trong collection để AI recommend khi khách gửi tin nhắn chứa từ khóa liên quan.
Ghi chú
Tính năng sẽ được mở rộng sau, cho phép merchant tự tạo collection và search term để thiết lập rule recommend linh hoạt hơn (cần thu thập nhu cầu từ khách)
...
### 101. [Improvement] Cải thiện hiển thị tin nhắn trên chatbox
...
• Render chat page vào vị trí mong muốn (merchant có thể chọn element cụ thể để hiển thị)
• Hỗ trợ Chatty SDK → cho phép trigger mở chat page từ các tương tác hoặc button tùy chỉnh
...
### 103. Bổ sung tình huống xử lý order tracking
...
• Guest (chưa đăng nhập): nếu đã nhập email/phone ở pre-chat form → AI xác nhận thông tin và chỉ hỏi thêm order number.
2. Kết hợp FAQs liên quan
• Với đơn hàng chưa giao (Unfulfilled) → AI hiển thị trạng thái đơn hàng và thời gian giao dự kiến (nếu có trong data source)
3. Cải thiện UI form tra cứu đơn hàng
...
### 104. [Improvement] Cập nhật filter trên inbox
...
• Processing: conversation AI đang xử lý (Assignee = AI, bao gồm transfer to human nhưng vẫn tiếp tục với AI)
• Handover: conversation đã được transfer sang human, cần xử lý tiếp
• Resolved: Conversation được xử lý hoàn toàn bởi AI
3. Channels
• Thêm tab Online store để lọc conversation từ Online store

**Task:** Thêm filter AI conversation (https://www.notion.so/Th-m-filter-AI-conversation-201b0da449f1800e87b3c68f6334227d?pvs=21)

---

### 105. [Improvement] Update banner liên hệ hỗ trợ AI training
...
Vị trí: AI assistant | Data source, Smart collection https://prnt.sc/zhBSO5BBSing

Mô tả tính năng
Thêm màn quản lý collection tại AI assistant | Data source
◦ Gồm 2 collection: Best Seller, New Arrival
◦ Activate smart syncing: bật/tắt chế độ sync tự động (cập nhật top 20 sản phẩm trong 30 ngày gần nhất, refresh hàng ngày lúc 12:00 AM)
◦ Cho phép thêm/xoá sản phẩm đã thêm (dù manual/auto sync) 
◦ AI sử dụng sản phẩm trong danh sách để gợi ý sản phẩm best seller, new arrival
...
### 107. Hiển thị giá sản phẩm theo domain
...
Mô tả tính năng
• Khi AI recommend sản phẩm trong chat: hiển thị giá sản phẩm dựa trên market và domain mà người dùng đang truy cập (từ catalog của market tương ứng) 
• Note: không hỗ trợ trường hợp quy đổi tỷ giá bằng app thứ 3
...
### 108. Recommend sản phẩm với thông tin đúng theo domain
...
• Map dữ liệu product với domain tương ứng để AI dùng khi trả lời
• Khi AI hiển thị kết quả sản phẩm trong response → URL luôn theo locale/domain hiện tại

[New Feature] AI gợi ý sản phẩm theo đúng Market & Domain
Vấn đề
• Khi AI recommend sản phẩm, hệ thống chỉ hiển thị tên và giá gốc, không phản ánh đúng giá theo market/domain mà khách đang truy cập.
• Với các store đa thị trường, AI vẫn trả về link mặc định, không theo domain/locale của khách → gây nhầm lẫn và trải nghiệm mua sắm không đồng bộ
Mục đích
• Hiển thị giá và link sản phẩm chính xác theo market và domain khách đang truy cập
• Đảm bảo trải nghiệm mua sắm thống nhất với store, không bị sai lệch giá hoặc domain
Mô tả tính năng
• Hiển thị giá sản phẩm trong chat dựa trên market/domain hiện tại (catalog của market tương ứng)
• Map dữ liệu sản phẩm với domain để AI luôn trả về link đúng theo locale/domain của khách
• URL và giá hiển thị trong response được đồng bộ theo market/domain khách đang dùng
• Lưu ý: không hỗ trợ các trường hợp dùng app thứ 3 để quy đổi tỷ giá
...
### 109. [New Feature] Button “View similar” trên Product Card
...
### 110. Thêm tin nhắn tự động sau khi submit pre-chat form
...
• Nội dung auto-reply tự động generate và chèn tên khách hàng nếu có trong form https://prnt.sc/aPrVgZmfx_4T
...
### 111. [Improvement] Điều chỉnh hiển thị App Embed Settings
...
• Khi bật:
    ...
    ◦ AI replies được tính vào AI reply usage của plan
    ◦ Product card hiển thị dưới dạng link khi gửi qua social
...
### 113. Thêm action Send transcription tại chatbox header
...
Mô tả tính năng
1. Chuyển form transcript thành button cố định trên chat header
    ◦ Nút Email transcript hiển thị rõ ràng, dễ tìm
    ◦ Có thể request bất kỳ lúc nào, không phụ thuộc trạng thái conversation
    ◦ Button sẽ disable khi chưa có tin nhắn nào từ user
2. User flow
    ◦ Click Email transcript tại header
    ◦ Mở modal nhập email
    ◦ Submit → hệ thống gửi email toàn bộ nội dung hội thoại (forward theo setting trong Email preference)
3. Ưu tiên CSAT survey
    ◦ Sau khi conversation resolve → hiển thị resolve alert + gửi form survey (không bị gián đoạn bởi transcript)
...
### 114. Embed chat page in storefront
...
• Customization options:
    ...
    ◦ Gồm các cài đặt khác: https://prnt.sc/3jpFzDWzRqEV
• Điều kiện hiển thị: bật App embed setting và Live chat
...
### 115. Cập nhật logic pre-chat form với Delay after X min
...
• Bỏ field: When the form appears, thay bằng field “Show form after X messages from visitors” https://prnt.sc/4RNMNeSeAnS2 (default = 3 message)
• Store cũ lựa chọn Show after X minute sẽ được cập nhật thành Chat as guest
...
• Thêm banner thông báo số lượng unresolved question tại trang Data source (chỉ hiển thị khi có > 1 unresolved question)
...
### 118. [Improvement] Cho phép điều chỉnh tag Best seller, New arrival cho AI recommend
...
### 119. [Improvement] Cập nhật logic Welcome message theo online status
...
2. Thêm tuỳ chọn gửi Welcome message khi offline https://prnt.sc/C7ynpm4dcRer
    ◦ Mặc định tắt (áp dụng cho cả store cũ và mới) 
    ◦ Khi tích: hiển thị thêm field Offline message → cho phép nhập nội dung welcome trong giờ offline
    ◦ Khi bỏ tích: Welcome message chỉ gửi trong online hour
...
https://prnt.sc/kpntUDJnz9oh
2. Welcome message: mặc định hiện ở Homepage, bỏ cài đặt Page to show https://prnt.sc/co2qVYuhT8KB
...
### 121. [New feature] AI replies usage history
...
◦ Free plan: giữ nguyên, thêm mục AI replies usage history (mặc định collapse) 
◦ Paid plan:
    ◦ Trang Subscription: chỉ hiển thị subscription details, AI replies usage history (mặc định expand), FAQ
    ◦ Thêm trang Plan details: các thông khác liên quan đến plan gồm: plan comparison, guarantee, testimonial, FAQs.
3. Cập nhật billing display
• Total charge (monthly + additional charge)
• AI replies: số lượng AI replies đã sử dụng/limit và additional charge
...
### 122. [Improvement] Cải thiện màn AI product management
...
• Mô tả:
    ...
    ◦ Thêm action Add product https://prnt.sc/EKvAJ3iBRKsD
        ▪ Sử dụng để thêm sản phẩm sử dụng cho AI training
        ▪ Chỉ hiển thị sản phẩm chưa sync
        ▪ Thông báo khi thêm vượt product limit
    ◦ Action "Sync products": chỉ cập nhật thông tin cho sản phẩm đã thêm
    ◦ Trạng thái sync: hiển thị số sản phẩm đã sync/tổng sản phẩm của store https://prnt.sc/LNWpWWoyXnV4
...
### 123. [Improvement] Thêm translation cho order status
...
### 124. [AI Assistant] Thêm FAQ cho từng product
...
• Tại trang Product:
    ◦ Cho phép thêm / quản lý FAQs cho mỗi sản phẩm
    ◦ Hiển thị số FAQs đã thêm tại List view và modal Product description
• Manage FAQs: Xem / thêm / sửa / xoá FAQs
2. AI Training
• AI được train với dữ liệu Product FAQs
• AI có thể sử dụng FAQs để trả lời chính xác hơn cho các câu hỏi liên quan đến từng sản phẩm cụ thể
...
### 125. [Improvement] Cập nhật Conversation Starter
...
2. Thêm validate:
Nếu bật Conversation starter nhưng chưa có câu hỏi → hiển thị cảnh báo 🔗 https://prnt.sc/fK5CInZJZBOs?utm_source=chatgpt.com
...
• Sau khi xoá:
    ...
    ◦ Với Customer → xoá conversation trong profile (giữ lại contact)
2. Xoá contact từ trang Contacts
• Thêm nút Delete tại cả List view và Edit profile https://prnt.sc/z0mKLWjMc8EP, https://prnt.sc/ufDvrQw6dU7A
• Nhấn Delete: xoá toàn bộ conversation liên quan đến contact đó

**Task:** Xoá conversation, contact (https://www.notion.so/Xo-conversation-contact-20db0da449f1805aaf12ed26f9a012a4?pvs=21)

---

### 127. [Improvement] Cải thiện giao diện quản lý Translation
...
### 128. [Improvement] Thêm tuỳ chọn bật/tắt Market sync
...
### 129. [Improvement] Thêm format tin nhắn tại các kênh Messenger, Instagram
...
### 130. Auto add to cart in chatbox
...
1. Tự động cập nhật cart 
    ...
    ◦ AI nhận intent thêm/sửa/xoá cart qua tin nhắn → tự động cập nhật cart
2. Sau khi cập nhật cart: gửi cart summary + hướng dẫn checkout
...
### 131. Index FAQs entries in Search engine
...
### 132. [New Feature] Quick button chuyển trạng thái Online/Offline
...
• Thêm cài đặt cho phép agent tự chuyển trạng thái Online/Offline
• Mặc định chuyển online khi vào app https://prnt.sc/PeJT6XQTOT3f
• Trạng thái này sẽ được dùng để
   • Xác định “Online status” hiển thị trên chatbox https://prnt.sc/SRhLB66NoC-A
   • Hiển thị trạng thái agent trong Inbox: giúp nhận diện agent đang online để assign phù hợp https://prnt.sc/x9tO25eZfnMm
...
### 134. [New feature] Welcome message theo online status
...
• Welcome message được tách thành 2 field:
    ◦ Online message
    ◦ Offline message
• Tin nhắn hiển thị theo working hour (cài đặt tại Settings | Chat  availability) và online status (tự động detect dựa trên trạng thái login vào app của agent)
2. Cập nhật 1st auto reply
• Thời gian gửi auto reply được đồng bộ theo Chatbox status
• Hiển thị chính xác theo trạng thái online/offline (trước đây chỉ gửi theo working hours)
...
### 136. Tạo mẫu instruction theo ngành hàng
...
2. Giảm giới hạn ký tự Custom instruction
    ◦ Giới hạn còn 1000 ký tự
    ◦ Điều chỉnh prompt AI để generate instruction ngắn gọn hơn
3. Cập nhật label các trường Instruction
    ◦ Làm rõ sự khác biệt giữa Custom instruction và Scenario instruction để tránh nhập sai nội dung
...
### 137. Đánh intent cho mỗi AI response
...
• Modal Review sources (cả ở Inbox và Test):
    ...
    ◦ Thêm action Add Q&As: Click mở modal trên trang hiện tại, pre-fill: question, answer, topic (intent) theo câu hỏi được thêm 
2. Unresolved Questions
• List view:
    ◦ Hiển thị intent của từng câu hỏi
    ◦ Thêm action Add answer / Ignore ngay ngoài bảng để thao tác nhanh hơn 
• Modal Review unresolved question: Pre-fill thông tin theo conversation 
3. Intent Analytics 
• Thống kê số lượng câu hỏi theo từng intent
• Hiển thị success rate của từng intent
https://prnt.sc/jjoenh716Vfc
...
### 138. [New feature] Satisfaction survey analytics
...
### 139. [New Feature] Nâng cấp Automation thành Campaign – Proactive chat theo hành trình khách hàng
...
⚙️ Mô tả tính năng
1. Danh sách campaign 🔗 https://prnt.sc/Ah3C4jx1jJ6s
• Pre-purchase: Welcome visitor, Subscribe newsletter, Product recommendation
• Engage: Cart booster
• Priority: lựa chọn thứ tự campaign hiển thị
2. Cài đặt chung
• Activate campaign: bật/tắt hiển thị trên storefront
• Tên campaign
• Trigger: thời điểm kích hoạt campaign (khác nhau tuỳ loại campaign)
• Send message on: chọn trang hiển thị campaign
• Conditions: Audience, Display time, Device, Display duration
• Message: Proactive message
• Tần suất hiển thị: 1 lần/session sau khi user click/close và ko hiện lại
3. Cài đặt riêng theo loại campaign
• Subscribe newsletter: chọn discount offer, cài đặt form subscribe và sucess message 
• Product recommendation: chọn loại sản phẩm gợi ý (Best seller / New arrival / Custom)
• Cart booster:
    ◦ Trigger: Minimum cart amount
    ◦ Message: Text / Product recommendation / Discount
4. Behaviour trên storefront
• Close teaser → Ẩn
• Click teaser → Gửi tin nhắn

Lưu ý với store cũ
• Hiển thị card giới thiệu campaign mới https://prnt.sc/YZ2RaOXGTF5S
• Nhấn “Try now” để test version mới
• Chỉ khi nhấn Publish mới ẩn automation cũ và áp dụng campaign mới https://prnt.sc/FG4fsgFH1Dn7
...
### 141. [Improvement] Cập nhật cài đặt giới hạn AI usage
...
• Thêm hiển thị current spent / limit, ước tính số AI replies theo spending limit 
https://prnt.sc/2F7wU_K1T3yx

2. Subscription details
• Cập nhật banner cảnh báo khi gần/đạt spending limit (tuỳ theo plan và Spending limit, thay vì warning theo monthly usage như trước)
• Thêm mục Monthly spending limit tại phần Additional AI replies
https://prnt.sc/5aAKhqGdeGCJ

3. Tự động bật AI khi upgrade
• Với Free: AI sẽ tự động tắt khi đạt limit
• Khi upgrade hoặc sang billing cycle mới → AI sẽ tự động bật lại

**Task:** Update logic AI usage limit (https://www.notion.so/Update-logic-AI-usage-limit-201b0da449f18040bbd2cda16e6dfa08?pvs=21)

---

### 142. Cải thiện gửi ảnh kèm văn bản tại inbox qua email
...
### 143. Send survey for AI conversation
...
### 144. Thêm card “Book call” tại trang AI Instruction
...
### 145. [Improvement] Cập nhật luồng hiển thị pre-chat form
...
### 146. [Improvement] Gộp First name, Last name  trên pre-chat, subscribe form
...
### 147. [New feature] Thêm filter tag, vendor, collection
...
### 148. [New feature] Cài đăt AI chỉ trả lời lúc agent offline
...
• Thêm cài đặt AI trả lời dựa trên thời gian online   
   • Always: mặc định AI trả lời kể cả khi agent online/offline
   • Only when agents are offline: AI chỉ trả lời khi agent offline (khi online conversation sẽ được assign theo assignment method)
• Khi trạng thái online/offline thay đổi, chỉ ảnh hưởng đến conversation mới (conversation đã được assign sẽ ko được tự động chuyển giữa AI và agent)
...
### 149. [New feature] Gợi ý giá sản phẩm theo market
...
• Sync dữ liệu theo thị trường cho AI training gồm:
    ...
    ◦ Product price: tính theo công thức của Shopify (chưa hỗ trợ custom price)
• Khi khách hỏi về giá sản phẩm / AI recommend sản phẩm, AI sẽ:
    ◦ Xác định thị trường đang truy cập
    ◦ Trả lời theo mức giá và đơn vị tiền tệ tương ứng
...
### 150. [New feature] Bật AI trả lời tự động trên email channels
...
### 151. [Improvement] Sync thêm thông tin product cho AI training
...
### 152. [Improvement] Update AI assistant layout
...
3. Cập nhật tab Instruction:
• Đổi tên từ Personality
• Bỏ các trường Tone of voice và Length of response → User có thể thêm vào General instruction
4. Test AI: chuyển nút Test lên góc phải để dễ truy cập hơna

**Task:** Update layout AI assistant (https://www.notion.so/Update-layout-AI-assistant-1cfb0da449f180998902d9dc9cf7470e?pvs=21)

---

### 153. Thêm proactive chat hướng dẫn thêm AI instruction
...
### 154. [New feature] Rate AI response
...
• Mô tả tính năng
    ...
    ◦ Analytics | AI Assistant: thêm các chỉ số: tổng AI reply, số lượng và tỷ lệ AI reply được đánh giá: Great/Good/Bad https://go.screenpal.com/watch/cTioQbnldDe
    ◦ Inbox: cho phép xem đánh giá từng reply của AI tại mỗi conversation (sau mình sẽ thêm bộ lọc Great/Bad reply để lọc nhanh những conversation có đánh giá tiêu cực)

**Task:** Rate AI response (https://www.notion.so/Rate-AI-response-199b0da449f180fdb546f4a170eb4a2b?pvs=21)

---

### 155. Scenario “Transfer to human”
...
• Mô tả tính năng
    ◦ Thêm scenario "Transfer to human agent", cho phép bật / tắt linh hoạt theo nhu cầu https://go.screenpal.com/watch/cTioQFnldDT
    ◦ Xác nhận trước khi chuyển agent: khi AI detect intent là talk to human, AI sẽ hỏi lại để xác nhận nhu cầu từ khách -> chỉ khi khách đồng ý AI mới transfer
...
### 156. [Improvement] Tối ưu luồng hiển thị Subscribe & Pre-chat – Tránh hiển thị chồng chéo khi mở chat
...
• Mặc định chỉ hiển thị form subscribe trước khi user mở chat
• Nếu user gửi tin nhắn (hoặc bỏ qua subscribe) → mới hiển thị pre-chat form
• Conversation chỉ được tạo sau khi user submit pre-chat
✅ Kiểm soát hiển thị & tạo dữ liệu:
• Nếu chưa submit pre-chat:
    ◦ Không tạo conversation
    ◦ Vẫn tạo contact ẩn danh (anonymous) nhưng không thể chỉnh sửa thông tin
• Nếu bật conversation starter:
    ◦ Hiển thị suggested FAQs mà không tạo conversation
    ◦ Chỉ khi user gửi tin nhắn → mới hiển thị pre-chat form
...
### 157. [Improvement] Thêm conversation history trên AI notification email
...
• Mô tả tính năng
    ◦ Bổ sung nội dung hiển thị trên email thông báo AI conversation gồm cả tin nhắn từ KH và AI
    ◦ Thêm AI name, customer name tại mỗi tin nhắn https://go.screenpal.com/watch/cTioQYnldDE
...
### 158. [Improvement] AI trả về đúng link sản phẩm theo từng variant
...
### 159. [New feature] Reset conversation
...
• Khi user nhấn "Reset conversation":
    ◦ Xóa toàn bộ lịch sử chat + context cũ tại FE
    ◦ Gửi lại welcome message, hiển thị suggested FAQs (nếu có)
2. Inbox
• Tự động resolve conversation cũ khi được reset 
• Tạo conversation mới sau khi reset, giữ nguyên profile khách hàng (name, email, tag...)

**Task:** [Chatbox] Reset conversation (https://www.notion.so/Chatbox-Reset-conversation-831b4c8b8b81474cad7b9a1eba651a28?pvs=21)

---

### 160. [Improvement] Update Scenario instruction
...
### 161. [New feature] AI hỗ trợ tra cứu trạng thái đơn hàng
...
Thêm sẵn một scenario “Order tracking” để AI tự động nhận diện và phản hồi câu hỏi liên quan đến đơn hàng
Luồng hoạt động
1. KH hỏi về trạng thái đơn hàng (AI tự detect intent) 
2. AI hỏi thông tin cần thiết để tra cứu (tuỳ theo cài đặt order tracking) 
- Cách 1. Order number + Phone/Email 
- Cách 2. Tracking number 
3. KH cung cấp thông tin
4. AI kiểm tra định dạng, yêu cầu bổ sung nếu thiếu hoặc sai
5. AI trả lời kết quả đơn hàng dưới dạng form, gồm 
- Thông tin cơ bản của order
- Thông tin tracking
6. Với đơn hàng chưa fulfilled hoặc ko tìm thấy → gợi ý liên hệ support


2. Cài đặt Order Tracking
• Chuyển cài đặt từ trang Chatbox sang Settings → Integration https://prnt.sc/563bI9QUw4VY
• Tại Chatbox, chỉ còn tuỳ chọn bật/tắt block https://prnt.sc/MkgVqrTtQjSf
• Cài đặt này dùng để: 
   • Hiển thị kết quả tra cứu tại block order tracking 
   • Làm dữ liệu cho AI trả lời tự động trong scenario Order tracking
...
### 162. Show chatbox in Checkout & Thank you page
...
• Chat được reset và tạo conversation mới khi người dùng mở chat tại 2 trang này, tách biệt với lịch sử chat trước đó (do limitation của Shopify)
...
### 163. Update proactive chat in app, focus on AI
...
• Card “Share your feedback”: chuyển từ trang General → Test, tự động gửi tin nhắn qua Crisp nếu user chọn mood Bad/Neutral
https://prnt.sc/ISxkodO_cfo8
• Tự động gửi hỗ trợ khi phát hiện hành vi cần trợ giúp:
    ◦ User click Reset test, hoặc gửi ≥ 3 tin nhắn liên tục
    ◦ AI trả về tin nhắn lỗi: No data source used, hoặc chỉ trả về message mặc định (chỉ gửi 1 lần/session) 
→ Nội dung tin nhắn: https://prnt.sc/8nItlGNUkaAL
...
### 164. [Improvement] Update customer profile edit, auto convert customer
...
• Yêu cầu nhập ít nhất 1 trong 3 thông tin https://prnt.sc/blhOojrXx8uK
2. Tự động gợi ý Convert to Customer/Create customer: 
• Khi chỉnh sửa email cho Lead hoặc Anonymous: hiển thị banner xác nhận convert customer https://prnt.sc/LzsWHGIBRq2S
...
### 165. [Improvement] Update transfer setting
...
• Pre-transfer information: Bỏ, do tỷ lệ sử dụng thấp, không có giá trị với người dùng
• Agent assignment: assign theo cài đặt Automation → Assignment method
• Show contact form:
    ◦ Bỏ cài đặt Collect info (mặc định chỉ collect email) 
    ◦ Tách phần Email support riêng để AI xử lý phù hợp khi unavailable
...
### 166. [Improvement] Tách trang Manage account
...
• Mô tả tính năng:
    ◦ Tạo mới màn Manage account https://prnt.sc/13Er2EcQSZEG
        ...
        ▪ Google login: hiển thị trạng thái connected nếu user đã log in bằng Google
    ◦ Màn Team: chỉ cho phép member xem thông tin, không chỉnh sửa profile tại trang này https://prnt.sc/AvNlrhlWK8S0
...
### 167. [New feature] Form hỗ trợ xử lý post purchase
...
• Mô tả tính năng: Thêm scenario mặc định về After sales service:
    ...
    ◦ Instruction: AI sẽ gửi policy liên quan và form request thông tin https://prnt.sc/DoncG4O_WG70
    ◦ Sau khi submit form:
        ▪ Handover theo transfer setting (ngoại trừ Contact form)
        ▪ Gửi thông tin submit form và transcript qua email để agent tiếp tục xử lý https://prnt.sc/V6KY7XgNzBEU
...
### 168. [Improvement] Cài đặt email nhận thông báo về budget
...
### 169. [Improvement] Update default chat page setting
...
### 170. [New feature] Detect intent transfer to human
...
Mô tả tính năng
• Thêm một scenario ẩn trong hệ thống để hướng dẫn AI tự động transfer khi phát hiện ý định của khách hàng (eg: I want to talk to a real person, Can I speak to a human agent?
• Hướng đến việc loại bỏ các button feedback sau mỗi câu trả lời AI và cho phép merchant tùy chỉnh thêm instruction riêng trong scenario để kiểm soát khi nào nên chuyển tiếp sang người
...
### 171. [New Feature] Hỗ trợ thiết lập số Zalo tùy chỉnh & Zalo Business
...
• Enable custom Zalo number: hỗ trợ nhập link liên hệ qua Zalo Business thay vì số điện thoại
(ví dụ: https://zalo.me/0123456789 hoặc link Zalo Official Account)

**Task:** Create enable custom zalo  (https://www.notion.so/Create-enable-custom-zalo-210b0da449f1804abf0ef08881ee5114?pvs=21)

---

### 172. [Improvement] Chặn gửi tin nhắn khi AI đang trả lời
...
### 173. [New feature] Gửi email thông báo về report trên app
...
• Bật/Tắt report: 
     ...
     • App performance report: cài đặt gửi email hàng tuần về hiệu suất sử dụng app
 • Tần suất gửi: 00:00 thứ 2 hàng tuần 
 • Nội dung email App performance report: gồm các chỉ số Overview và AI performance

**Task:** Email reports in app (https://www.notion.so/Email-reports-in-app-1afb0da449f180f6919efc1d78428244?pvs=21)

---

### 174. [Improvement] Cập nhật pre-chat form dạng inline
...
TH1. Chat as guest
・Hiển thị pre-chat form cùng welcome message 
・Ẩn ô chat, user phải điền form mới có thể gửi tin nhắn https://prnt.sc/bIi24_tfhU2k
TH2. Chat as anonymous (giữ nguyên)
・Không hiển thị form khi bắt đầu chat 
TH3. Show both option
・Trigger hiện form theo cài đặt When the form appears (after delay time/message) https://prnt.sc/pZ_r5uRKi9JN
・Click Chat as anonymous hoặc Send new message = Skip form (tạo conversation với type = Anonymous) 
・Submit form = Start chat (tạo conversation với type = Lead)

2. Thêm tuỳ chỉnh thông điệp tại form pre-chat 
・Trước đây: user phải chỉnh sửa ở translation gây bất tiện 
・Thay đổi: thêm field Message tại cài đặt pre-chat, cho phép nhập nội dung hướng dẫn để khách hàng hiểu lý do điền thông tin https://prnt.sc/8_Hjqc334yTQ
...
### 175. [Pre-chat form] Show form sau một vài message/certain time
...
• Mô tả tính năng
    ◦ Tuỳ chọn thời điểm hiển thị pre-chat form:
        ...
        ▪ Sau một khoảng thời gian nhất định (after delay): default = 0 minute
        ▪ Sau số lượng tin nhắn gửi bởi visitor (after X messages): default = 3 message
    ◦ Hành vi khi có AI response / automated message: form sẽ hiển thị sau khi AI hoặc hệ thống gửi phản hồi đầu tiênLưu ý: do luồng trước đây là sau khi submit pre-chat form mới hiển thị conversation tại inbox, luồng mới sẽ tạo conversation ngay khi KH gửi 1 tin nhắn → với khách cũ sẽ hiển thị những conversation cũ bị ẩn do chưa submit pre-chat tại inbox -> CS nếu nhận được feedback của khách thì có thể hướng dẫn khách resolve đi nhé ạ
...
• Trigger gửi khảo sát:
    ◦ Khi conversation được đánh dấu Resolved
    ◦ Khi phát hiện keyword khớp2. Giao diện khảo sát phía khách hàng https://prnt.sc/v_apk5QxRVHK
• Khách có thể chọn đánh giá hoặc bỏ qua
• Sau khi gửi, hiển thị thông báo “Thank you” và không thể chỉnh sửa lại phản hồi
• Khảo sát được gửi sau khi hoàn thành luồng Conversation history email (nếu bật)
...
### 177. [Growth hacking] Mở data source limit & offer discount theo tệp khách hàng
...
- Áp dụng cho tất cả plan (trừ Free và Plus)
- Khi đạt giới hạn data source, hiển thị banner cảnh báo cùng button “Get more usage” → User click để mở Crisp chat và prefill “I would like to get more data source” 
- CS hỗ trợ mở tại DEV ZONE https://prnt.sc/Jy-69O9ZM-Kx
Lưu ý:
- URL, File, QnA: có thể offer mở x2 usage và mở unlimited cho khách 
- Product: mở tuỳ theo plan và cần báo PM, chi tiết tại https://www.notion.so/1d6b0da449f1809292aac0076e385a08?pvs=21
...
Tại inbox:
- Tin nhắn từ WhatsApp hiển thị trực tiếp tại Inbox https://prnt.sc/lH9a8jwr4-x_
- Tự động tạo lead profile trong trang Contacts
- Merchant có thể trả lời trong vòng 24h từ thời điểm nhận tin nhắn cuối từ khách. Sau 24h, cần sử dụng WhatsApp template để tiếp tục gửi tin nhắn https://prnt.sc/9TxOYo3Neo--
...
### 179. [Improvement] Tích hợp Shopify MCP storefront
...
• Các kịch bản sử dụng MCP
    ...
    ◦ Policy Information (đang check lại thông tin lấy được): hỗ trợ trả lời các câu hỏi liên quan đến store policy mà không cần thêm vào data source
    ◦ Add to Cart: AI tự động thêm, sửa hoặc xoá sản phẩm trong giỏ hàng theo intent của khách, đồng thời cung cấp link checkout tương ứng
Lưu ý: Cart được tạo thông qua session riêng nên chỉ khi KH nhấn vào link checkout trong tin nhắn mới thấy item trong cart, mở cart trực tiếp trên store sẽ ko thấy
...
### 180. [New feature] Manage account in web app
...
### 181. [Improvement] Hỗ trợ mailto ở tin nhắn chứa email
...
### 182. [Improvement] Optimize dashboard setup guide
...
### 183. [New feature] Manual summarize chat
...
### 184. [New feature] Conversation analytics
...
### 185. Generate instruction by AI
...
4. AI generate instruction dựa trên mô tả business theo template có sẵn và cho phép chỉnh sửa 
5. Click Save để Insert instruction

Video: https://somup.com/cTh06CM5RX
...
### 186. [Improvement] Sắp xếp layout Automation, Chatbox settings
...
### 187. [Improvement] Cải thiện layout Overview analytics
...
### 188. [Improvement] Recommend product variant with lowest price
...
### 189. Record conversation test in merchant’s store
...
### 190. [New feature] Reply new and best seller product
...
### 191. [New feature] Sync metafield product cho AI training
...
・Các loại metafield hỗ trợ: tất cả trong https://help.shopify.com/en/manual/custom-data/metafields/metafield-definitions/metafield-typestrừ Reference type, Link, URL  ) 
・Tần suất sync: sync product sẽ lấy lại thông tin sản phẩm bao gồm cả product metafield 

・AI response: Trả lời được câu hỏi liên quan đến product dựa trên metafield
...
### 192. [New feature] Thêm cài đặt bắt buộc cho thông tin trên pre-chat form
...
• Mô tả tính năng:
    ◦ Thêm cài đặt "Show contact form", cho phép bật/tắt button Contact form trên chatbox khi khách hàng lựa chọn transfer to human https://prnt.sc/WFAt7pEcqZXP
    ◦ Nội dung contact form: https://prnt.sc/kqVhGW39G3wo
        ▪ Issue: vấn đề mà KH gặp phải
        ▪ Email/Phone (auto fill với guest/customer): thông tin liên hệ của KH
    ◦ Sau khi KH submit contact form, gửi email cho merchant (theo cài đặt trên app) gồm thông tin trên contact form và lịch sử chat https://prnt.sc/80oVqy6b_ct_
...
### 194. [New feature] Refresh URL, Preview content từ URL
...
### 195. [New feature] Mặc định mở cửa sổ chat khi mở chatbox
...
### 196. Train AI with store’s page URL
...
• Thông tin sync: 
   ...
   • Chỉ sync các trang phổ biến thuộc faq, shipping-policy, return-policy, privacy-policy ,terms-of-service, contact-us, about-us
• Action liên quan:
   • Sync store URL: cho phép làm mới toàn bộ dữ liệu từ các trang đã được sync từ Shopify (chỉ áp dụng các trang có trạng thái active)→  Limit 10 lần sync/ngày 
   • Re-sync URL: cho phép làm mới dữ liệu của mỗi từng URL 

Lưu ý: đối với các store đã sử dụng AI Assistant từ trước, cần tắt và bật lại AI Assistant để bắt đầu quá trình đồng bộ trang store
...
### 197. [Improvement] Cập nhật order status trên trang Order tracking
...
Sau thay đổi
Order status: bổ sung thêm trạng thái và hiển thị theo thứ tự ưu tiên (ưu tiên trạng thái cuối cùng có ảnh hưởng đến KH nhiều nhất): Returned → Canceled → Partially refunded/Refunded/Voided → Fulfillment status
Item status: cập nhật đầy đủ trạng thái item mỗi khi fulfill theo API hỗ trợ của Shopify
Translation: các trạng thái về đơn hàng sẽ được tự động dịch, các field dịch trước đó sẽ bị loại bỏ
...
### 198. [New feature] Review source khi test AI
...
　・Show list data sources/scenarios sử dụng, link redirect tới data sources/scenario
　・Button Add QnAs: click thì mở modal add QnAs, prefill question
　・Button Add other data sources: click thì redirect to Data sources

2. Inbox/Modal review sources
- Thêm btn Add QnAs: click thì mở modal Add QnAs, prefill question giúp thêm data source nhanh hơn

**Task:** Review sources khi test AI (https://www.notion.so/Review-sources-khi-test-AI-1c3b0da449f1805fa5d2c5207945d4f5?pvs=21)

---

### 199. [New feature] AI instruction by scenarios
...
• Mặc định thêm 2 scenario Product recommendation và Order return & Refund (store cũ để trống)
...
### 200. [New feature] Upload category icon
...
### 201. Cải thiện chatbox để đảm bảo accessibility
...
### 202. Thêm thông báo về AI completion rate
...
### 203. Cài đặt usage charge limit
...
### 204. Feature request by feedbear
...
### 205. Feature request by feedbear
...
### 206. [New feature] [AI assistant] Bulk delete, set active/inactive
...
### 207. [Improvement] Transfer admin to other member
...
### 209. [New feature] Resize chat zone
...
### 210. [Improvement] Tăng bảo mật khi download app
...
### 211. [Improvement] Improve chat performance
...
### 213. Thêm analytics về revenue trên dashboard
...
### 214. [Notifications] Thêm event “New message from AI conversations”
...
### 215. Sửa UI product card ở AI replies
...
### 216. [Improvement] Xử lý với open conversation khi tắt AI assistant
...
Sau cập nhật: Khi tắt AI hoặc AI bị tắt khi đạt limit usage với bản Free, những conversation do AI đang xử lý có trạng thái Open sẽ:
    ◦ Inbox: ngừng trả lời bởi AI khi có tin nhắn mới, tự động transfer agent tương ứng theo cài đặt tại AI Assistant > Transfer settings, show thông báo conversation được transfer to human
    ◦ Chatbox (FE)
        ▪ Gửi thông báo để KH biết cuộc hội thoại đang/sẽ được tiếp tục bởi con người,  https://prnt.sc/9MA_YPBB4jiP
        ▪ Ẩn lựa chọn “Continue with AI” nếu conversation đã được transfer to human
...
### 217. [Improvement] Bổ sung context cho AI với link gửi tại chatbox
...
### 218. [Inbox] Thêm review data source (URL, File)
...
### 220. [Improvement] Auto sync products after onboarding
...
• What:
    ...
    ◦ Cập nhật: sau khi user chọn plan tại bước 4 mới sync products theo limit của plan đã chọn
...
• What:
    ...
    ◦ Resolution rate: tỷ lệ phần trăm cuộc hội thoại có trạng thái ”Resolved”, kèm chỉ số phụ thể hiện số lượng conversation được resolved trên tổng số conversation
    ◦ Assisted revenue, Time save (coming soon) - cho thấy impact của app đến hiệu quả kinh doanh
...
### 222. [Inbox] Update list view Your inbox, Unassigned, AI
...
### 223. [Email channel] Giới hạn connect email ở Free plan
...
### 224. Lỗi Outlook email channel bị chặn forward
...
### 229. Update luồng tạo conversation
...
Update tracking order status: Thêm status Attempt delivery/ Delivered khi track order https://prnt.sc/pKRmRTUJ6LV8
...
### 232. Support pasting & sending images
...
• Thêm action download image https://prnt.sc/M6znfY83aoWz
...
### 233. Update translation settings
...
• Update ‘Set As Default Language’ action: Khách có thể change default bằng cách chọn 1 ngôn ngữ -> Mark as default https://prnt.sc/LF7ZEkHmvZ3I
• Khách hàng có thể remove unpublished lanague, ngôn ngữ được removed có thể add lại khi click Add language
• Update sắp xếp bản dịch giao diện theo Category tính năng https://prnt.sc/5oWOTCh2mSIb https://prnt.sc/eQgKsbEhbwOF, giúp khách dễ tìm và dễ hiểu hơn
• Gom navigation translation vào một category, chỉ apply khi bật Navigation https://prnt.sc/IhmMgZn16Efz
...
### 235. [Email channel] Gộp email khi reply
...
• Tại chatbox (FE): Khách hàng chưa để lại email sẽ cần điền email sau đó confirm, khách đã có chỉ cần confirm để gửi email sumup https://prnt.sc/VtBDoqaA_KiC
• Khi gửi email, khách hàng có thể unsubscribe email sumup tại footer của email đó https://prnt.sc/p_743o7mGwJI https://prnt.sc/KooS4VAiXPEN
...
### 237. Update font & style trên admin
...
### 238. Keep last chat screen & activity
...
### 240. Open & Share chat thông qua standalone
...
### 243. Include/Exclude pages/URL to display chatbox widget
...
### 244. Specific offline status

**Date:** August 6, 2024

---

### 245. Improve customer profile
...
### 246. Public live chat features for all customer, no more Beta version
...
• Tại inbox -> Survey form: Update label https://take.ms/HjDXO
...
### 247. Xoá image, file ở chat inbox và Shopify files
...
• Forward email tới khách hàng thông qua Chatty khi reply tại inbox https://prnt.sc/4FlA8Pn3JxcJCustomer:
• Có thể contact merchant thông qua email, tạo conversation mới và merchant đồng thời nhận email https://prnt.sc/Jp85zS6qpnOE
• Merchant cần enable Settings -> Follow-up email for customer https://prnt.sc/fxeQio2ZXzolThêm hướng dẫn setup:
• Tại integration: thêm Email channel (Ảnh)
• Khách hàng cần follow theo các step setup tại guide để follow (mn nên thử để nắm rõ cách setup)
...
### 255. Improvement: Cập nhật banner refresh order
...
### 256. Manage profile guest/anonymous
...
• Update thông tin và matching nếu email trùng với customer
• Tạo customer nếu thông tin email không trùngKhi convert thành customer, hiện activity tại chat https://i.imgur.com/zvY7LUO.png
...
### 257. Sửa lỗi update customer profile
...
• Ngôn ngữ của FAQs sẽ không cập nhật gì, status là Updated (Viết FAQs bằng Tiếng anh thì bản dịch Tiếng Anh không thay đổi)
• Các ngôn ngữ khác để Outdated (như cũ) -> Click auto translate để dịch
• Nếu ngôn ngữ FAQs là ngôn ngữ default:
• Chỉ update status là Updated https://i.imgur.com/Y12jwo4.png
• Các ngôn ngữ khác để Outdated (vẫn như cũ) -> Click auto translate để dịch

Sửa lỗi không gửi được discount https://somup.com/cZi3juGhWm

---

### 261. Online/offline status
...
• Với Anonymous: Mở modal điền email khi detect khách hàng offline để đảm bảo anonymous nhận được tin nhắn https://i.imgur.com/kKckrMx.png
...
• Với contact method đã add mà chưa có value, hiện thông báo https://imgur.com/Gy3qyTK. Khách hàng cần phải fill thông tin vào contact method thì mới show được chat button

---

### 263. Insert FAQs in chat
...
### 266. Resolved/unresolved conversation:
...
### 267. Update Subscribe newsletter chatbot
...
### 269. Subscribe newsletter chatbot improvements
...
• Bật livechat: Hiển thị Yes/No để khách hàng lựa chọn giữa subscribe form hoặc chat tiếp https://i.imgur.com/81zr3wu.png
• Khi khách hàng hoàn thành chatbot, 1 conversation sẽ được tạo
• Thêm design cho darkmode của subscribe form https://i.imgur.com/4qf6bXY.png
• Sửa lại session hiển thị: từ 1 lần/session (1 session = 1 lần mở trình duyệt) -> 1 session = 30 ngày
...
### 270. Gửi tin nhắn cho customer qua email (hidden feature)
...
• Hiện chưa detect được status Online/Offline của Customer nên có thể thử ở dev zone -> enable online status https://i.imgur.com/gsMP7v2.png
...
### 272. Block customer & Filter blocked
...
### 273. Update Preheader email notification
...
### 274. Order info in conversation
...
### 275. Convert anoymous & guest to customer
...
### 276. Play sound notification ở admin và storefront
...
• Hiển thị số tin nhắn chưa đọc ở icon https://i.imgur.com/DbFgcta.png
• Khi merchant/customer hoạt động ở tab khác, tab bar vẫn nháy thông báo https://i.imgur.com/phYO8Do.png
...
• Thêm Install mobile app step ở Setup guide: https://i.imgur.com/VwnyC5c.png
• Thêm banner ở Settings: https://i.imgur.com/k8rUO0S.png
...
### 279. Sửa logo Chatty in app & top save bar
...
### 280. Thêm count unread conversation in menu
...
### 281. Thêm số unread conversation trên menu

**Date:** June 27, 2024

---

### 282. Sửa logo Chatty khi reload, đổi top save bar lên trên header
...
### 284. Update expiration cho chat session của guest và anonymous:
...
### 285. Search conversation by name, email
...
### 287. Thêm settings disclaimer consent
...
### 288. Search conversation, Update expiration
...
### 289. Assistant -> Subscribe newsletter chatbot:
...
• Success: Tin nhắn khi subscribe thành công https://prnt.sc/JQ0D1OY6g_RrKhi customer điền email, chatbot sẽ tự động tạo customer trên Shopify và đánh dấu Email subscribed https://prnt.sc/55725eH2ZSe8Khi enable cùng Welcome chatbot, luồng Subscribe newsletter sẽ được ưu tiên hơnKhi bật livechat, chat sẽ được tạo khi customer subscribe thành côngKhi không bật livechat, chat sẽ không được tạo. Và chatbox cũng ẩn khi hết luồng chatbot https://prnt.sc/9Ev1CihuL2iS
...
### 290. Thêm unread status và filter unread
...
### 291. Hiển thị thông tin Customer & Orders tại Conversation details
...
### 292. Copy text ở admin & front end
...
### 293. Cho phép dịch Question & Answer tại FAQs:
...
• Với những ngôn ngữ chưa được publish, khi publish lên cũng yêu cầu update FAQs trước khi update https://prnt.sc/MCYorJ_lqPPR
• Khách hàng có thể update tay hoặc ấn Auto translate sẽ ẩn các banner và thông báo update FAQs
...
### 294. Show customer activities tại store ở admin chat
...
### 296. Cải thiện giao diện front end chatbox
...
### 302. Update luồng order tracking
...
### 303. Cho phép điều chỉnh độ rộng cột Conversation list và conversation details
...
### 304. [Livechat] Cài đặt thông báo tin nhắn mới
...
### 305. Update app theo polaris 12
...
### 309. Move FAQs sang partner Chatty
...
### 310. [Translation] Bổ sung dịch label, ngôn ngữ phổ biến
...
### 312. [Widget] Bổ sung widget display style & Mobile widget ratio
...
### 313. [FAQ page] Update category display
...
### 314. Cập nhật user guide trên Avada Helpdesk center
...
### 315. [Chat] More widget button type
...
### 316. [Onboarding+Dashboard] Cập nhật checklist status trên Dashboard khi hoàn thành onboarding
...
### 317. Bổ sung translation cho widget và FAQ page
...
### 318. [Widget] Update widget launcher
...
### 319. [FAQ page] Update category display
...
### 320. Update BFS mandatory @January 8, 2024
...
### 322. Update settings, add welcome popup & chat availability functions

**Date:** November 16, 2023 9:00 AM (GMT+7)
...
### 323. Block conversation từ channels (email, messenger, instagram)

**Date:** April 2, 2025 → December 31, 2025
...
**Date:** April 1, 2025 → December 31, 2025
...
### 325. Preview scenario Transfer to human

---

### 326. Update AI limitation mới
